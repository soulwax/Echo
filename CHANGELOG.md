...
- Added a `/download` command to download a track to discord. Provided by an own API endpoint using a different project that is private, legal and for commercial purposes. The API is not open source and is not available to the public.

- Added a `/youtube` command to post a youtube video link to discord. This uses yt-dlp. First it attempts to get the video, using ffmpeg, to the 50MB size limit, then 10MB, if that doesn't work, it will just post the link.

- Various improvements picking up from <hi@maxisom.me>.
