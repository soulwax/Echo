...
export default class DownloadCommand implements Command {
  ...
  private async downloadFile(
    ...
    // Default filename in case content-disposition is not available
    ...
    if (contentDisposition) {
      ...
      if (match) {
        filename = match[1].replace(/['"]/g, ''); // Remove any enclosing quotes
      }
    }
    ...
    // Make sure the filename only ends with mp3
    const fixedFilename = filename.replace(/\.mp3+$/, '.mp3');
